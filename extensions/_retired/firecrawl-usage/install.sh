#!/usr/bin/env bash
# Installs the Firecrawl Usage GNOME Shell extension for the current user.
#
...
run cp "$REPO/schemas/org.gnome.shell.extensions.firecrawl-usage.gschema.xml" \
  "$DEST/schemas/org.gnome.shell.extensions.firecrawl-usage.gschema.xml"
...
History: ~/.cache/firecrawl-usage/history.jsonl
Auth:    ~/.config/firecrawl-cli/credentials.json (firecrawl login)
EOF
