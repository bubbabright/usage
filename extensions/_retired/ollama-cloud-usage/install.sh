#!/usr/bin/env bash
# Installs the Ollama Cloud Usage GNOME Shell extension for the current user.
#
...
run mkdir -p "$DEST/schemas" "$DEST/assets"
...
run cp "$REPO/schemas/org.gnome.shell.extensions.ollama-cloud-usage.gschema.xml" \
  "$DEST/schemas/org.gnome.shell.extensions.ollama-cloud-usage.gschema.xml"
run cp -r "$REPO/assets/." "$DEST/assets/"
